...
const VoyageCard = ({ data, isLoading, beforeData } : any) => {
    if(isLoading){
        return(
            <React.Fragment>
                {[...Array(beforeData)].map((_, idx) => (
                    <Card key={idx} className="p-2 h-full w-full" radius="lg">
                ...
                {data?.map((voyage : any , index : number) => (
                    <Card key={index} className='h-full w-full' isHoverable isPressable>
                        <label className='h-full w-full p-2 text-center flex flex-col justify-center gap-2  cursor-pointer'>
                            ...
                            <input type="radio" value={JSON.stringify(voyage)} name='voyage_code' className='hidden'/>
                        </label>
                    </Card>
                ))}
            </React.Fragment>
...
const Voyages = () => {
    ...
    return (
        <div className="voyages h-[500px] mt-10 w-full flex flex-col gap-y-7 md:flex-row md:h-40 md:gap-x-7">       
            <VoyageCard beforeData={beforeData} data={data} isLoading={isLoading} />
        </div>
    )
}
